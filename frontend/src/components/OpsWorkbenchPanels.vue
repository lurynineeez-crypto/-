...
const summaryCards = computed(() => {
  if (props.activeView === 'environment') {
    return [
      ...
      { label: '可见点位', value: `${props.displaySensors.length} 个`, note: '仅来自现有硬件清单' },
  ...
  if (props.activeView === 'control') {
    return [
      ...
      { label: '目标', value: '稳态恢复', note: '看动作、对象和复核结果' },
      { label: '复核点', value: '10 分钟', note: 'CO2 与湿度变化复查' }
    ];
  }
  if (props.activeView === 'alarms') {
    return [
      { label: '报警总数', value: `${props.alarmTimeline.length} 条`, note: '当前队列' },
      { label: '待处理', value: `${props.alarmTimeline.filter((item) => !item.handled).length} 条`, note: '点击进入详情' },
      { label: '闭环要求', value: '处理 + 复核', note: '最终进入报告追溯' }
    ];
  }
  return [
...
const alarmRows = computed(() => props.alarmTimeline.map((alarm, index) => ({
  ...alarm,
  point: alarm.sensorId,
  currentValue: index === 0 ? '84.9%RH' : index === 1 ? '1720ppm' : '关注',
  target: index === 0 ? '目标 ≥85%RH' : index === 1 ? '目标 500-1500ppm' : '按阶段目标复核',
  status: alarm.handled ? '已闭环' : index === 0 ? '待复核' : '待处理',
  suggestion: index === 0
    ? '17:30 前现场复核并记录'
    : index === 1
      ? '新风联动后 10 分钟复查回落曲线'
      : '按 SOP 分派处理并提交复核',
  owner: index === 0 ? '种植员' : '运维员',
...
<template>
  <section class="ops-workbench-layout focused-workbench">
    ...
    <div v-else-if="activeView === 'alarms'" class="ops-workbench-grid alarm-focus">
      <PanelSection
        :icon="ShieldAlert"
        title="报警队列"
        description="报警页面只做处置：确认、分派、复核、闭环。"
        wide
      >
        <div class="ops-alert-list priority-alert-list">
          <button
            v-for="alarm in alarmRows"
            :key="alarm.id"
            type="button"
            class="ops-alert-row closed-loop-alert"
          ...
          >
            ...
            <div class="alarm-field-grid">
              <span>点位：{{ alarm.point }}</span>
              <span>当前值：{{ alarm.currentValue }}</span>
              <span>目标：{{ alarm.target }}</span>
              <span>责任人：{{ alarm.owner }}</span>
              <span>更新：{{ alarm.updatedAt }}</span>
            </div>
            <footer>
              <small>{{ alarm.suggestion }}</small>
              <em>报警 → 分派 → 处理 → 复核 → 关闭</em>
            </footer>
            <div class="alert-action-row">
              <span>建议动作：{{ alarm.suggestion }}</span>
        ...
        <div class="ops-alert-summary">
          <article>
            <span>报警总数</span>
            <strong>{{ alarmTimeline.length }}</strong>
          </article>
          <article>
            <span>未闭环</span>
            <strong>{{ alarmTimeline.filter((item) => !item.handled).length }}</strong>
          </article>
          <article>
            <span>优先级</span>
            <strong>预警</strong>
